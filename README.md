...
Determination of background and laser sheet: use 4 background images (fIn2: no dye, light on; fIn3: no dye, light off; fIn4: dye present, light on; fIn5: dye present, light off) to estimate: the camera’s own noise; the spatial distribution of the laser sheet; and the response when the dye is uniform (used for scaling).
...
5. df_dfm_read 可以理解成：把 .dfm 电影文件里的若干帧，真正读成 MATLAB 里的图像数组的函数。
...
Note: the code for generating the coordinate system is in DigiFlow.
